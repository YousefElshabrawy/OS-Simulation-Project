...
char *memAlgo;
int memory[1024];
bool firstFitAllocate(int size,int *start,int *end){
    int count=0;
    for(int i=0; i<20;i++){
        if(memory[i]==0) {
            count++;
...
bool nextFitAllocate(int size, int begin,int *start,int *end){
    int count=0;
...
void bestFitAllocate(int*memory, vector *vec,int size, int*start, int e) {
    int count = 0;
    ...
    for (int i = 0; i < 30; i++) {
        if (memory[i] == 0) {
            ...
            if (count == size) {
                ...
                Pair p =  Pair__create(count,start);
                vec->pfVectorAdd(vec,p);
                vec->vectorList.items[0].lower_bound = vec->vectorList.items[0].lower_bound - count ;
                vec->vectorList.items[0].upper_bound = e+1;
                return;
            }
        }
        else count = 0;
...
void bestFitDeallocate(int* memory, vector *vec,int size)
{
    for (int i = 0; i < (int)vec->vectorList.total; i++)
    {       
        if (vec->vectorList.items[i].lower_bound == size)
        {
            int start = vec->vectorList.items[i].upper_bound;
            for (int i = start; i < start+size; i++)
            {
                memory[i] = 0;
            }
            vec->vectorList.items[0].lower_bound = vec->vectorList.items[0].lower_bound + size;   
        }   
    }   
}
int main(int argc, char *argv[])
{
    for(int i =0 ; i<1024;i++) memory[i]=0; //initializing 
    signal(SIGINT, handler);
    initClk();
    algo = argv[1];
    //memAlgo = argv[3];
    memAlgo="1"; //just for test
    printf("from the scheduler the mem algo is %s\n",memAlgo);
...
void FCFS(char* algo){
    ...
    while((ptr=receiveMessage(msgq_id))!=NULL ){
        ...
        if(prc != NULL){
                ...
                process_num++;
                int start,end;
                /*if(firstFitAllocate(ptr->memsize,&start,&end)){

                }*/
                printf("the memory size is = %d ",ptr->memsize);
                int pid;
                pid = fork();
                if(pid==-1)
                    ...
                    compileAndRun("process.out" , "process" , "process.out" ,algo  , NULL , NULL);
                }
                else{
...
void SJF(char* algo){
    ...
    while(!(lastprocess && isEmpty(p_queue))){
        ...
        struct process* prc =get_first(p_queue);
        if(prc){
                fprintf(memory_log , "At time %d process %d memory %d \n" , getClk() , prc->id  , prc->memsize);
                wait_time_sum = wait_time_sum + getClk() - prc->arrival;
                total_running = total_running + prc->runtime;
                process_num++;
                int pid;
...
int startProcess(struct process *prc)
{
    ...
    if (prc->pid == 0) // start a new process
    {
        ...
        compileAndRun("process.out", "process", "process.out", algo, quatom, NULL);
    }
