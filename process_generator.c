...
int main(int argc, char *argv[])
{
    ...
    if(file == NULL){
        ...
        last->memsize=memsize;
        last->runtime=runtime;  
        last->remaining_time=runtime;  
        last->last_process=0;  
        last->last_in_second=0;
        ...
        while (temp<numberOfProcesses-1)
        {
            ...
            ptr->memsize=memsize;
            ptr->remaining_time=runtime;  
            ptr->last_process=0;  
            ptr->last_in_second=0;
    ...
    char* algo = argv[2];
    char* memAlgo = argv[3];
    printf("from the pg the mem algo is %s\n",memAlgo);
    char* quantom = NULL;
    if(strcmp(algo,"5")==0)
    {
        quantom = argv[3];
    ...
    if(pid==-1)
        ...
        compileAndRun("clk.out" , "clk" , "clk.out" , NULL , NULL , NULL);
    }
    else{
            ...
            if(pid == 0){
                ...
                //scheduler_id = getpid();
                compileAndRun("scheduler.out" , "scheduler" , "scheduler.out" , algo , quantom , memAlgo);
            }
            else if(pid == -1){
